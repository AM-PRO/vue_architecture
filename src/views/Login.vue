  ...
  <div>
    <Card title="Connexion">
      <form @submit.prevent="login()" class="flex flex-col gap-5 items-center">
        ...
        <ErrorText class="error-login" v-if="isError">{{ error }}</ErrorText>
      </form>
    </Card>
    <Card title="Informations">
      <ul class="list-disc">
        <li>
          Vous pouvez vous connecter avec "jhon.doe@gmail.com" et "password"
          pour avoir accès à la todolist
        </li>
        <li>
          Il n'y a pas de page register mais cela fonctionnerait de la même
          manière
        </li>
        <li>
          Je n'ai pas implémenté de vrai service d'authentification étant donné
          que le serveur ne propose pas de route
        </li>
      </ul>
    </Card>
  </div>
